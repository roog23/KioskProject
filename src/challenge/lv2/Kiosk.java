...
public class Kiosk {
    ...
    public void start() {
        ...
        while (true) {
            ...
            int categoryCheck = printErrorCheck("category");
            ...
            //장바구니 기능을 선택한 경우 실행됩니다.
                ...
                if(categoryCheck- menu.getCategorySize()==1) {
                    ...
                    if(calculate.equals("1")){  //1을 입력받은 경우만 주문을 진행합니다.
                        //세일 정보를 출력하고, 입력에 오류가 있는지 확인합니다.
                        int saleInfo = printErrorCheck("saleInfo");
                ...
                //장바구니 목록을 선택한 경우 실행됩니다.
                else if(categoryCheck- menu.getCategorySize()==3) {
                    String calculate = scanner.next();
                    if(calculate.equals("1")){  //1을 입력받은 경우만 장바구니 메뉴 취소를 진행합니다.
                        int removeMenu = printErrorCheck("removeMenu");
            ...
            int menuCheck = printErrorCheck("menu");
    ...
    public int printErrorCheck(String list) {
        int check;
        ...
        while(true) {
            ...
            if(list.equals("category")) {
                menu.printCategory();
                if (cart.getCartSize() != 0) {  //장바구니에 메뉴가 들어가 있는 경우 입니다.
                    cart.printOrder(menu.getCategorySize());
                }
            }
            // 메뉴 목록을 출력해줍니다.
            else if (list.equals("menu")) menu.printMenu();
            // 할인 정보를 출력해줍니다.
            else if (list.equals("saleInfo")) {
                System.out.println("\n할인 정보를 입력해주세요.");
                SaleInfo.printSaleInfo();
            }
            // 취소할 메뉴를 입력을 안내해줍니다.
            else if (list.equals("removeMenu")) {
            ...
            if(list.equals("category")) {
                if (cart.getCartSize() == 0 &&  //장바구니에 메뉴가 없는 경우 처리 방식입니다.
                    ...
                    continue;
                        ...
                        (check > menu.getCategorySize() + 3 || check < 0)) {
                    System.out.println("카테고리 옆에 작성된 숫자를 입력해주세요.\n");
                    continue;
                }
            }
            ...
            //할인 목록에 지정된 숫자 외의 입력이 들어오는 경우 처리 방법입니다.
            if(list.equals("saleInfo") && (check > 4 || check < 1)){
                System.out.println("할인 목록에 등록된 숫자를 입력해주세요.");
                continue;
            }
            //장바구니에 등록된 메뉴에 지정된 숫자 이외의 입력이 들어오는 경우 처리 방법입니다.
            if(list.equals("removeMenu") && (check > cart.getCartSize() || check < 1)){
                System.out.println("메뉴 옆에 작성된 숫자를 입력해주세요.");
                continue;
            }
